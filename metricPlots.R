...
####created by Madeline Melzer 20230731, updated by Madeline E Melzer 20240117
...
getwd()
dataDirectory <- "/Volumes/fsmresfiles/Basic_Sciences/CDB/CDB_Collaborations/Arispe_Goyal/MadelineMelzer/DATA/nucleiDysmorphia/data/age_final/results/metricTables/"
saveDirectory <- "/Volumes/fsmresfiles/Basic_Sciences/CDB/CDB_Collaborations/Arispe_Goyal/MadelineMelzer/DATA/nucleiDysmorphia/data/age_final/results/umapTables/"
plotDirectory <- "/Volumes/fsmresfiles/Basic_Sciences/CDB/CDB_Collaborations/Arispe_Goyal/MadelineMelzer/DATA/nucleiDysmorphia/plots/final/"

### loading the data
data = as_tibble(read.table(paste0(dataDirectory, "20240117_allAges.csv"), stringsAsFactors=F, header = T, sep = ","))

### applying area threshold
lower_bound =  5502.375
upper_bound =  17819.375
data = data %>% filter(area >= lower_bound & area <= upper_bound & location != "" & !is.na(pointRadiusRMSDifference)) #added lower and upper bounds from 1.5 IQR area, cross check with metrics.ipynb result

### processing data for dimensional reduction with PCA and UMAP
data <- data %>% mutate(ID=row_number()) #the ID is to link the UMAP data with the metadata
...
#data.pca.meta = as.data.frame(metrics.pca$x) %>% inner_join(as.data.frame(metadata), by = character(0))
#write.csv(data.pca.meta, paste0(saveDirectory, "20240115_pca.csv"))
...
write.csv(data.umap.df, paste0(saveDirectory, "20240117_umap_allMetrics.csv"))
...
### counting number of cells for each age, aortic location
